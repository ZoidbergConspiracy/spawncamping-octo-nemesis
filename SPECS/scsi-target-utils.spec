...
BuildRequires:  pkgconfig libxslt docbook-style-xsl systemd-units libaio-devel
%if 0%{?with_rdma}
...
%{__make} %{?_smp_mflags} %{?with_rdma:ISCSI_RDMA=1} CEPH_RBD=1 GLFS_BD=1 libdir=%{_libdir}/tgt
...
pushd usr
%{__make} install %{?with_rdma:ISCSI_RDMA=1} CEPH_RBD=1 GLFS_BD=1 DESTDIR=%{buildroot} sbindir=%{_sbindir} libdir=%{_libdir}/tgt
